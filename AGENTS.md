...
- `extensions/` typescript extensions (tools, commands, ui, hooks), auto-discovered
  ...
  - (deleted) `clear-on-shrink.ts` persisted `terminal.clearOnShrink=true` for pi 0.80.7, where a shrink left a stale blank row. pi 0.84.2 answers that flag in `clearStatusIndicator()` by parking a 2-line `IdleStatus` in the dock, so the workaround now costs two permanent blank rows under the transcript. removed, along with the live flip in `footer.ts` and the persisted key
  - `halfblock-boxes.ts` replaces the blank `paddingY` rows of every `Box` with half-height block characters (`▄` on top, `▀` on the bottom, drawn in the box's own background colour), so a tool bubble costs no blank rows. also drops the blank lines a tool row wraps itself in, and the leading `Spacer(1)` of an assistant message when a tool row is what precedes it
