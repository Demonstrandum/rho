...
- `extensions/` typescript extensions (tools, commands, ui, hooks), auto-discovered
  ...
  - `spinner.ts` sets the working indicator and message, driven by `spinners.json` and `maxims.txt`
...
- `spinners.json` spinner definitions with `enabledCategories` (default `chinese`)
- `maxims.txt` working messages, one per line, picked at random each turn
