...
the kernel is one thread, and code-mode runs a batch's cells before its exit returns, so a slow cell would hold every call: code sent while a cell runs is declined after a short wait rather than queued (a dropped request interrupts the kernel, cell and all), the table while a cell runs is answered from the feed with how long it has been at it, and an edit batch answers after `[colab] wait-seconds` with its request left open behind it, so the cells finish and `marimo_run` with `interrupt` is the way to stop them.
an image never travels through the kernel's stdout: marimo caps console output, a PNG in base64 is a megabyte, and a session was once shown the truncated middle of one as an error. the helper writes images (cell outputs, mimebundles, screenshots) to files under the temp directory and the tool reads and removes them, and a failure text replaces any run of base64 with a marker before it is shown.
a screenshot needs playwright in the kernel's interpreter; when it is missing the tool installs it with `uv pip --python`, not with marimo's installer, which in a uv project is `uv add` and edits the manifest and lock for a renderer the notebook does not import.
a project that has lived with marimo has a layer of its own (a module that wraps `marimo._code_mode` in named calls, a skill, a launcher, widgets, a notebooks directory), and an agent arriving with generic tools once drew a figure beside such a layer instead of extending its widget; `lib/colab/project.ts` finds the layer by content, `marimo_open` reports it and prints the module's `state()` when it has one, and the skill says to read and act through it first.
...
the nine tools are not carried by every session: they are taken out of the loadout at `session_start`, before the first request, and put back when something asks for them, which is a message naming marimo, a notebook, colab or an .ipynb, `/colab`, a `read` of `skills/marimo/SKILL.md` or of a file that builds `marimo.App`, or a resume that rejoins notebooks.
this is the pattern the environment and remote tools use, moved one event earlier: a hide on the first `tool_call` lets the first request of every session carry the definitions, and here that is nine of them.
`skills/marimo/SKILL.md` carries the graph rules and the conventions, loaded on demand, and says in its first lines that reading it loads the tools; the tool descriptions carry the two rules that cost the most turns when missed (one owner per name; edit through the kernel, not the file).
