...
```
/environment connect samuel@dev-box-1
```

Then ask for `hostname`, `pwd`, a file read, a file edit. All of it should
happen on dev-box while the session stays on the laptop.
...
```
/remote create mock samuel@dev-box-1
/remote connect mock
```

After `connect` this terminal is a viewer: what you type goes to the session on
dev-box, and its replies are drawn here.

- `/remote disconnect`, then `/remote list samuel@dev-box-1`. The session
  is still running.
...
```
/remote project git@github.com:example-org/demo-server.git demo/main samuel@dev-box-1
/remote connect demo
```

Makes `~/projects/demo/checkout/demo-server` and
