...
- robotics-vm, always up, cheap, holds the agent and its history.
...
```
/remote create mysesh samuel@robotics-vm
...
**Measured against robotics-vm**, from a host with nothing installed on it: create 5.6s, list 0.4s, attach 0.7s warm, an answer streaming 2.3s after asking.
...
a socket here and the ledger of what was started on another host were each read by the extension that wrote them, so `pi --attach overnight` answered for a session on robotics-vm with "no session called overnight here".
