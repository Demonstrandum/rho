...
const SCHEMA = {
    ...
    colab: {
        runner: field(
            'runner',
            isOneOf('auto', 'marimo', 'uv', 'uvx'),
            'auto',
            'which marimo starts a notebook server. auto prefers a .venv with marimo,',
            'then uv run in a project that names marimo, then marimo on PATH, then',
            'uvx. the others force one of those',
        ),
        port: field('port', isPosInt, 2718, 'first port tried for a server this session starts; the next free one is used'),
        sandbox: field(
            'sandbox',
            isOneOf('auto', 'always', 'never'),
            'auto',
            'pass --sandbox, which builds the notebook its own environment from its',
            'inline (PEP 723) dependencies. auto does so when the file carries them',
        ),
        mcp: field(
            'mcp',
            isBool,
            true,
            'start servers with --mcp, so other agents can attach to the same kernel',
            'over http at <url>/mcp/server. needs the marimo[mcp] extra; a server',
            'without it prints a warning and runs on',
        ),
        browser: field('browser', isBool, true, 'open the notebook in the browser when /colab attaches to it'),
        keep: field(
            'keep',
            isBool,
            false,
            'leave servers this session started running when the session ends,',
        ...
        runOnOpen: field(
            ...
            isBool,
            true,
            'run every cell when a notebook is opened without a session, so the',
            'first look at it shows outputs and errors. off leaves the cells unrun',
        ),
        outputChars: field(
            'output-chars',
            isPosInt,
            4000,
            'longest text a single cell output or stdout is passed to the model at;',
