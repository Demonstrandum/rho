...
const SCHEMA = {
    ...
    render: {
        ...
        boxToolTails: field(
            ...
            isBool,
            true,
            "pi's edit tool draws its own row, and puts the text of a failed or",
            'changed result outside it, on an unpainted line after a blank one.',
            'wrap that text in a box of its own, so it takes the same background',
            'and the same edges as every other row',
        ),
