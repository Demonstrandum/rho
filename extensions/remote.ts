 ...
 *
 * robotics-vm's 1.3.13 dies inside undici on pi 0.85.1, with an error about
...
export default function (pi: ExtensionAPI) {
    ...
    const create = async (name: string, address: string, say: (note: string) => void): Promise<string> => {
        ...
        const started = await run(
            'ssh',
            [...SSH_FLAGS, host, posix(`${withPi}${runner} serve ${name} ${address_.path ?? '$HOME'}`)],
            lend(),
        );
        await run('ssh', [...SSH_FLAGS, host, posix(linkRunner)]);
        if (started.code !== 0) {
    ...
    const ask = async (host: string, verb: string, say: (note: string) => void): Promise<string> => {
        ...
        const runner = await place(host, say);
        const again = await run('ssh', [...SSH_FLAGS, host, posix(`${runner} ${verb}`)]);
