...
robotics-vm is always up and cheap, so it holds the session and its history.
...
```
symba create --profile cpu-small --name work --time 2
symba list                                   # for the tailnet address
```

then `environment connect ubuntu@<address>`, do the work, and
`symba terminate <node id> --yes` when it is done.
...
state that must survive goes on robotics-vm or in a repository, never on a
rented node.
