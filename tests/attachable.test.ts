...
describe('where a name is', () => {
    ...
    test('a name in the ledger is the machine the ledger says', () => {
        writeLedger({ hosts: new Map([['overnight', 'samuel@robotics-vm']]), worktrees: new Map() });
        expect(attachable('overnight')).toEqual({
            where: 'remote',
            name: 'overnight',
            host: 'samuel@robotics-vm',
            path: null,
        });
    });
    ...
    test("a project's session carries the worktree it was made in", () => {
        writeLedger({
            hosts: new Map(),
            worktrees: new Map([['rho-master', { host: 'samuel@robotics-vm', path: '/home/samuel/projects/rho' }]]),
        });
...
describe('the names that can be typed', () => {
    test('both records are offered, each said where it is, and no name twice', () => {
        ...
        expect(offers()).toEqual([
            ...
            { name: 'overnight', where: 'remote', detail: 'samuel@robotics-vm' },
...
describe('the ledger the two extensions share', () => {
    test('what one writes the other reads', () => {
        writeLedger({
            hosts: new Map([['overnight', 'samuel@robotics-vm']]),
            worktrees: new Map([['rho-master', { host: 'samuel@robotics-vm', path: '/home/samuel/p/rho' }]]),
        });
        const read = readLedger();
        expect(read.hosts.get('overnight')).toBe('samuel@robotics-vm');
        expect(read.worktrees.get('rho-master')).toEqual({ host: 'samuel@robotics-vm', path: '/home/samuel/p/rho' });
    });
    ...
    test('the flat name-to-host file the first version wrote is read as the sessions it was', () => {
        ...
        writeFileSync(join(where, 'sessions.json'), '{"overnight":"samuel@robotics-vm"}\n');
        expect(readLedger().hosts.get('overnight')).toBe('samuel@robotics-vm');
    });
...
describe('connecting to another machine', () => {
    test('what /remote publishes is what the flag calls', async () => {
        ...
        await connect?.({} as never, 'overnight', 'samuel@robotics-vm');
        expect(called).toEqual(['overnight@samuel@robotics-vm']);
    });
});
