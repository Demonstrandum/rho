...
const HOSTS = [{ value: 'samuel@robotics-vm' }, { value: 'nix@robotics-vm' }];
...
describe('completing it', () => {
    test('keeps everything typed before the word', () => {
        ...
        expect(found?.map((item) => item.value)).toEqual(['project repo.git feature/remote samuel@robotics-vm']);
        // and the list still reads as a list of choices
        expect(found?.map((item) => item.label)).toEqual(['samuel@robotics-vm']);
    });

    test('offers everything when the word is empty', () => {
        const found = completeLastWord('connect ', HOSTS);
        expect(found?.map((item) => item.value)).toEqual(['connect samuel@robotics-vm', 'connect nix@robotics-vm']);
    });

    test('matches without regard to case', () => {
        expect(completeLastWord('SAM', HOSTS)?.map((item) => item.value)).toEqual(['samuel@robotics-vm']);
    });
